...
def test_product_3(product_3):
    assert product_3.name == "Iphone 14 pro max"
    assert product_3.get_price() == 35000.0
    assert product_3.description == "amaled display, 8/512Gb , 50Mp ,Color-Purple"
    assert product_3.quantity == 5
...
def test_category_1(category_1):
    assert category_1.name == "Телефоны"
    assert category_1.description == "Самые топовые делефоны до 35000"
    assert category_1.get_products() == [
        "Samsung Galaxy S22 Snapdragon 8 gen 1, 35000.0 руб. Остаток: 3 шт.",
        "Honor 200 lite, 30000.0 руб. Остаток: 6 шт.",
        "Iphone 14 pro max, 35000.0 руб. Остаток: 5 шт.",
    ]
...
def test_category_get_products(category_1):
    assert category_1.get_products() == [
        "Samsung Galaxy S22 Snapdragon 8 gen 1, 35000.0 руб. Остаток: 3 шт.",
        "Honor 200 lite, 30000.0 руб. Остаток: 6 шт.",
        "Iphone 14 pro max, 35000.0 руб. Остаток: 5 шт.",
...
def test_product_set_price(product_1):
    product_1.set_price(40000.0)
    assert product_1.get_price() == 40000.0

    product_1.set_price(0)
    assert product_1.get_price() == 40000.0

    product_1.set_price(-100.0)
    assert product_1.get_price() == 40000.0
